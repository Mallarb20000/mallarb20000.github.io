...
const getBandColorClasses = (score: number): string => {
  if (score >= 7) return 'border-green-600 bg-green-50 dark:bg-green-900 text-green-700 dark:text-green-300'
  if (score >= 5) return 'border-amber-600 bg-amber-50 dark:bg-amber-900 text-amber-700 dark:text-amber-300'
  return 'border-red-600 bg-red-50 dark:bg-red-900 text-red-700 dark:text-red-300'
}
...
const BandScoreCard: React.FC<{ 
  ...
  return (
    <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-slate-200 dark:border-gray-700 shadow-sm h-full flex flex-col">
      <div className="flex items-start gap-4 mb-4">
        <div className={`flex-shrink-0 w-12 h-12 rounded-lg flex items-center justify-center ${colorClasses.split(' ')[1]} ${colorClasses.split(' ')[2]}`}>
          {icon}
        </div>
        <div>
          <h4 className="font-bold text-slate-800 dark:text-gray-100">{title}</h4>
          <p className={`text-2xl font-bold ${colorClasses.split(' ')[2]} ${colorClasses.split(' ')[3] || ''}`}>Band {score}</p>
        </div>
      </div>
      <p className="text-slate-600 dark:text-gray-300 text-sm leading-relaxed flex-grow">{justification}</p>
    </div>
  )
}
